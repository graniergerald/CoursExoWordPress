...
class ExtractCriticalCss extends CallbackBase
{
	...
	function processMatches( $aMatches, $sContext )
	{
		...
		{
			if ( ! $this->bProcessFontFace )
			{//If we're not processing font-face yet let's just save them for later until after we've done getting all the
				// critical css
			...
			else
			{
				...
				if ( ! empty( $aM[1] ) && strpos( $this->sCriticalCss, $aM[1] ) !== false /*&& ! in_array( $aM[1], $this->aFonts )*/ )
				{
					//$this->aFonts[] = $aM[1];
